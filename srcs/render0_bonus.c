...
t_wall			check_hit(double fdr[6], t_pos idr[2], t_data *data)
{
	...
	while (1)
	{
		...
		if (data->map[(int)mod(idr[POS].x, data->smap.x)][(int)mod(idr[POS].y,
					data->smap.y)] == MAPWALL)
		{
			res.dist = fdr[res.side ? DISTX : DISTY];
			res.x = res.dist * fdr[ORIX] + mod(data->x, data->smap.x);
			res.y = res.dist * fdr[ORIY] + mod(data->y, data->smap.y);
			return (res);
		}
